...
from src.augmentation import get_train_transforms, get_valid_transforms
...
def set_train_dataloader(
    ...
    num_workers: int = 4,
    split: bool = True,
    trn_idx: List[int] = None,
    val_idx: int = None,
):
    """Prepare train and valid dataloaders with the use of split or fold
    ...
    if split:
        train_ = df[df.split == 0].reset_index(drop=True)
        valid_ = df[df.split == 1].reset_index(drop=True)
    else:
